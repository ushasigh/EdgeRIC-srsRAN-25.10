                                                  ...
                                                  span<ue_newtx_candidate> ue_candidates)
{
  ...
  for (ue_newtx_candidate& candidate : ue_candidates) {
    ...
    // EdgeRIC: Weight-based scheduling now handled in grant_params_selector.cpp
    // Do NOT multiply priority by weight here - it creates a feedback loop with PF
    // where high-PRB allocations increase avg_rate, decreasing future priority.
    // 
    // Only block scheduling if weight is explicitly 0
    uint16_t rnti = static_cast<uint16_t>(candidate.ue->crnti());
    std::optional<float> edgeric_weight = edgeric::get_weights(rnti);
    
    if (edgeric_weight.has_value() && edgeric_weight.value() <= 0.0f) {
      // Weight of 0 means no allocation - set priority to 0
      pf_metric = 0.0;
    }
                                                  ...
                                                  span<ue_newtx_candidate> ue_candidates)
{
  ...
  for (ue_newtx_candidate& candidate : ue_candidates) {
    ...
    // EdgeRIC: Weight-based scheduling handled in PRB allocation, not priority
    // Only block scheduling if weight is explicitly 0
    uint16_t rnti = static_cast<uint16_t>(candidate.ue->crnti());
    std::optional<float> edgeric_weight = edgeric::get_weights(rnti);
