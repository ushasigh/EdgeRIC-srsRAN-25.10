...
void scheduler_time_qos::save_dl_newtx_grants(span<const dl_msg_alloc> dl_grants)
{
  ...
  for (const dl_msg_alloc& grant : dl_grants) {
    // Skip grants with empty codewords (failed allocations)
    if (grant.pdsch_cfg.codewords.empty() || grant.tb_list.empty()) {
      continue;
    }
    ...
    for (logical_channel_config_ptr lc : *u.logical_channels()) {
      ...
      if (policy_params.pdb_enabled) {
        ...
        if (hol_toa.valid() and slot_tx >= hol_toa) {
          ...
          delay_weight += hol_delay_ms / static_cast<double>(lc->qos->qos.packet_delay_budget_ms);
        }
      }

      if (not lc->qos->gbr_qos_info.has_value()) {
        // No GBR config - skip GBR calculation
        continue;
      }
