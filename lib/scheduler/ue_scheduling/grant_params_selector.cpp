...
// EdgeRIC: Track unused CRBs at the start of each TTI for weight-based allocation
// Uses edgeric::tti_cnt directly to detect TTI changes (matching ue_cell_grid_allocator logic)
static uint32_t count_tti = 0;
static unsigned this_tti_unused_crbs = 0;
                                                            ...
                                                            unsigned                      pending_bytes)
{
  ...
  for (unsigned pdsch_td_index = 0, e = ss.pdsch_time_domain_list.size(); pdsch_td_index != e; ++pdsch_td_index) {
    ...
    ctxt.rnti               = ue_cc.rnti();
    ctxt.total_bwp_rbs      = crb_lims.length();
    return ctxt;
  }
...
{
  // Start with the calculated required PRBs (from MCS/TBS computation)
  unsigned calc_prbs = decision_ctxt.expected_nof_rbs;
  unsigned nof_rbs_to_allocate = calc_prbs;
  
  // Get total VRBs in BWP and calculate currently available
  unsigned total_vrbs = decision_ctxt.total_bwp_rbs > 0 ? decision_ctxt.total_bwp_rbs : used_vrbs.size();
  unsigned used_vrbs_count = used_vrbs.count();
  unsigned unused_crbs = total_vrbs - used_vrbs_count;
  
  // At TTI boundary, capture the total available PRBs for this TTI
  // This matches the ue_cell_grid_allocator logic where the first UE sets this_tti_unused_crbs
  if (count_tti != edgeric::tti_cnt) {
    this_tti_unused_crbs = unused_crbs;
    count_tti = edgeric::tti_cnt;
  }
  
  // Apply EdgeRIC weight-based allocation if weight is available for this UE
  // Uses staleness check: weights expire after STALENESS_THRESHOLD TTIs if muApp stops
  if (decision_ctxt.rnti != rnti_t::INVALID_RNTI) {
    uint16_t rnti = static_cast<uint16_t>(decision_ctxt.rnti);
    std::optional<float> opt_weight = edgeric::get_weights(rnti);
    
    if (opt_weight.has_value()) {
      float weight = opt_weight.value();
      
      // Weight-based allocation: PRBs = weight * this_tti_unused_crbs
      nof_rbs_to_allocate = static_cast<unsigned>(weight * static_cast<float>(this_tti_unused_crbs));
      
      // Cap to the calculated required PRBs (don't allocate more than needed)
      nof_rbs_to_allocate = std::min(nof_rbs_to_allocate, calc_prbs);
      
      // Ensure at least 1 PRB if weight > 0 and there's available space
      if (weight > 0.0f && nof_rbs_to_allocate == 0 && unused_crbs > 0) {
        nof_rbs_to_allocate = 1;
      }
    }
  }
  
  // Apply max_nof_rbs limit
  nof_rbs_to_allocate = std::min(nof_rbs_to_allocate, max_nof_rbs);
  
  // Find available VRBs
  vrb_interval result = rb_helper::find_empty_interval_of_length(used_vrbs, nof_rbs_to_allocate);
